...
const BuyerProductDetails = () => {
  ...
  // Accept auctionId from navigation state if available (passed from BuyerHomePage/BuyerProductCard)
    ...
    const timer = setInterval(async () => {
      ...
      if (isUpcomingOver) {
        if (!auctionStartHandled.current) {
          ...
          // Fetch the auction record and update status to ONGOING if needed
          await fetchAuctionDetails();
          if (auction && auction.currStatus === 'SCHEDULED') {
            try {
              const token = getAuthToken();
              // Send all required fields for backend validation
              await axios.put(
  ...
  const getProductInfo = () => {
    ...
    return {
      ...
      priceJump: product.priceJump || 1000, // Default price jump
            ...
            <div className="flex items-center gap-4 mb-5 pb-4 border-b border-[#393f4d]">
              ...
              <div className="flex-1">
                ...
                <div className="flex gap-6 text-xs opacity-80 mt-1">
                  ...
                  {/* <span>Status: {productInfo.productStatus}</span> */}
                </div>
              </div>
            </div>
