...
import ferrariImg from '../../assets/Data/1969_Dino_246_GT/FERRARI-Dino-246-GT-1072_14.jpeg';
...
const SellerHomePage = () => {
  ...
  try {
    ...
    // console.log('Request URL:', `${SERVER_URL}/products/seller/${sellerId}`);
    ...
    // The backend returns the products array directly
  ...
  const getImageUrl = (imageData) => {
    if (!imageData) return ferrariImg;
          ...
          {auctionItems.map((item, index) => (
            <div key={item.productId || index}>
              ...
              <div className={styles.imageContainer}>
                <img
                  src={item.mainImageUrl ? getImageUrl(item.mainImageUrl) : (item.additionalImageUrls && item.additionalImageUrls.length > 0 ? getImageUrl(item.additionalImageUrls[0]) : ferrariImg)}
                  alt={item.productModel || item.title}
                  onError={(e) => {
                    console.log('Failed to load image for product:', item.productId);
                    e.target.src = ferrariImg;
                  }}
                />
              </div>
              ...
              {/* {item.productStatus === 'DECLINED' && (
                <div className={styles.btnReview}>
                  <button className={`${styles.value} ${styles.review}`} onClick={handleRevSnd}>Add Review</button>
                </div>
              )} */}
              <button 
